...
<?php
    ...
    $result = mysqli_query($con, $sql);//mysqli_connect를 통해 연결된 객체를 이용해서 쿼리문을 실행해서 결과값을 저장한다.
    $total_record = mysqli_num_rows($result); // 전체 글 수 //그 레코드셋의 로우 수를 세어주는 것.
...
?>
			...
			<ul id="page_num">
<?php
    if ($total_page>=2 && $page >= 2) {//전체 페이지가 2페이지 이상 잇으면 어떤 페이지를 갈 수 있고, 현재 페이지가 2페이지 이상이면 이전 페이지를 갈 수 있다.
        $new_page = $page-1;
    ...
    if ($total_page>=2 && $page != $total_page) {		//토탈페이지가 2페이지보다 크고 현재 페이지가 토탈페이지 크기 위치가 아니면 다음 페이지를 찍어준다.
        $new_page = $page+1;
